...
#         self.linear = nn.Linear(512*block.expansion, num_classes)
        self.linear = nn.Linear(in_features=204800, out_features=5, bias=True)
...
def ResNet50_train(net,num_train=28099):
    true_ans = 0.0
    false_ans = 0.0
    confusion_yTrue = []
    confusion_yPred = []
...
def run(net,n=10):
    ...
    test_epoch_list = []
    test_acc_list = []
    f= open('ResNet50_pretrain.txt','a')
    for epoch in range(10):
        ...
        result = ResNet50_train(net,len(train_data))
        # result = ResNet50_train(net,n)
